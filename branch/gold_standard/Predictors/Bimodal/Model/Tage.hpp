// From https://github.com/Hiren6/TAGE-Branch-Predictor/
#include <bitset>
...
void BitsFolding(fold_history_t *f, GHIST ghist) {
    ...
    // Last bit in history with the modulus somewhere
...
void gold_standard_predictor::impl_initialise(void) {
    uint8_t cpu = 0;
...
uint8_t gold_standard_predictor::predict_branch(uint64_t PC){
    uint8_t cpu = 0;
...
void gold_standard_predictor::impl_last_branch_result(uint64_t ip, uint64_t target, uint8_t taken, uint8_t branch_type) {
    ...
    uint8_t cpu = 0;
    // CHANGE
    uint8_t predicted = predict_branch(ip); // CHANGE
    ...
    if ((predicted != outcome) && (pt > 0)) {
        ...
        // CHANGE a lot of this implementation
        // Common case - Mispredictions are unlikely, so rather than multiple ports just stall on that case for allocation
        if (found_empty == true) {
            int random_num = rand() % 100; // CHANGE How to do this in hardware, also for gold standard I need to change this to something deterministic
    ...
    if (pt == NOT_FOUND) {  // if no tag matches in any table then use default bimodal
        ...
        // CHANGE - this does not seem correct, where is the alt pred?
