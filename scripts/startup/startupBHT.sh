...
for ((i=10; i<=10**7; i*=10)); do
    ...
    echo $BASE_COMMAND "--simulation_instructions" 1000 ${TRACEDIR}${TRACE} "&> results/startup/bht/${i}.txt"
    $BASE_COMMAND "--simulation_instructions" 1000 ${TRACEDIR}${TRACE} > "results/startup/bht/${i}.txt"
done
