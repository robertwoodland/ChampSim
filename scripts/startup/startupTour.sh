...
for ((i=10; i<=10**7; i*=10)); do
    ...
    echo $BASE_COMMAND "--simulation_instructions" 1000 $TRACE "&> results/startup/tour/${i}.txt"
    $BASE_COMMAND "--simulation_instructions" 1000 $TRACE > "results/startup/tour/${i}.txt"
done
